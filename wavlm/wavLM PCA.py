...
EXCEL_DIR="Excels"
WAV_DIR="wav files"
MODEL_DIR="artifacts_pca" # Changed dir to avoid overwriting LDA artifacts
TEST_SIZE=0.2
RANDOM_STATE=42
PROGRESS_EVERY=20
USE_AMP=True
...
Xr=pca.fit_transform(X) # PCA is unsupervised, so we only pass X
...
joblib.dump(pca,os.path.join(MODEL_DIR,"pca.joblib")) # Save PCA
...
def load_artifacts(model_dir=MODEL_DIR,device_str=None):
    ...
    pca_=joblib.load(os.path.join(model_dir,"pca.joblib"))
    svm_=joblib.load(os.path.join(model_dir,"svm.joblib"))
    return fe,mdl,enc,pca_,svm_,dev

@torch.no_grad()
def predict_one(audio_path,model_dir=MODEL_DIR):
    fe,mdl,enc,pca_,svm_,dev=load_artifacts(model_dir)
